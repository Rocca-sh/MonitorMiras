...
public class OrgServImp implements OrgServ {
    ...
    public Org createOrg(Org org) {
        ...
        org.setSipId(SipCreator.genOrgSip(nextId));
        org.setPasswd(passwordEncoder.encode(org.getPasswd()));
        return orgPg.save(org);
    }

    @Override
    public Org getOrgBySipId(String sipId) {
        return orgPg.findById(sipId)
                .orElseThrow(() -> new NotFoundException("Empresa con SIP " + sipId + " no encontrada"));
    }
}
