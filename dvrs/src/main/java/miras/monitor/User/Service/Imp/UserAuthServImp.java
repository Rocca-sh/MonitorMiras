...
public class UserAuthServImp implements UserAuthServ {
    ...
    public User createUser(User user) {
        ...
        if (user.getOrganization() != null && user.getOrganization().getUlid() != null) {
            if (!orgPg.existsById(user.getOrganization().getUlid())) {
                throw new NotFoundException("El ID de la empresa no existe");
            }
            user.setRole("OWNER");
        } else {
            user.setRole("VIEWER");
    ...
    public User linkUserToOrg(User linkData, String code) {
        ...
        Org org = orgPg.findById(linkData.getOrganization().getUlid()).orElseThrow(() -> new NotFoundException("Empresa no encontrada"));
